...
const testConfig = {
    ...
    cert: path.resolve(ROOT_PATH, 'ssl/Swish_Merchant_TestCertificate_1234679304.pem'),
    key: path.resolve(ROOT_PATH, 'ssl/Swish_Merchant_TestCertificate_1234679304.key'),
...
export const config = process.env.NODE_ENV === "development" ? testConfig : prodConfig;

const agent = new https.Agent({
    cert: `${fs.readFileSync(config.cert, { encoding: 'utf8' })}`, // public key
    key: fs.readFileSync(config.key, { encoding: 'utf8' }), // private key
    ca: config.ca ? fs.readFileSync(config.ca, { encoding: 'utf8' }) : null, // root cirtificate optional
