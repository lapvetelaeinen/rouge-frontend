...
const SwishPayment = (props) => {
  ...
  return (
    <div
      className={`bg-neutral-800 absolute z-50 h-full w-full flex justify-center items-start bg-opacity-80 ${styles.paymentWrapper}`}
    >
      <div
        className={`bg-neutral-200 w-full min-h-[700px] m-4 rounded-3xl ${styles.paymentModal}`}
      >
        <div className="flex flex-col">
          ...
          {callbackUrl && (
            <div className={styles.paymentStatusLinkBlock}>
              <p className={styles.paymentStautsLinkInfo}>
                Klicka på knappen när du betalat i Swish-appen
              </p>
              <a
                href={callbackUrl}
          ...
          {error && (
            <div className={`${styles.paymentError} ml-4`}>{error}</div>
          )}
        </div>
      </div>
    </div>
